...
export class GithubUtility extends Utility implements RepoInterface {
  ...
  public async outputBranch(): Promise<void> {
    this.output(
      `Current Branch: ${colors.green(await this.getCurrentBranch())}`,
    );
  }
  ...
  public async push(): Promise<string> {
    return this.run(
      `git push --follow-tags origin ${await this.getCurrentBranch()}`,
    );
  }
  ...
  public async outputPull(): Promise<void> {
    this.output(
      `${colors.green(await this.getCurrentBranch())}: ${await this.pull()}`,
    );
  }
  ...
  public async outputCreateBranch(
    branchName: string,
    from?: string,
  ): Promise<void> {
    this.output(await this.createBranch(branchName, from));
  }
  ...
  protected async getCurrentBranch(): Promise<string> {
    return (await this.run(`git rev-parse --abbrev-ref HEAD`)).replace(
      /\r?\n|\r/g,
      '',
    );
  }
